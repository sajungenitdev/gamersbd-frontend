...
interface WishlistContextType {
    ...
    moveToCart: (itemId: string, quantity?: number, platform?: string) => Promise<boolean>;
...
export const WishlistProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const addToWishlist = async (productId: string, note?: string): Promise<boolean> => {
        if (!token) {
    ...
    const removeFromWishlist = async (itemId: string): Promise<boolean> => {
        ...
        try {
            const response = await axios.delete(`${API_URL}/api/wishlist/remove/${itemId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const moveToCart = async (itemId: string, quantity: number = 1, platform?: string): Promise<boolean> => {
        if (!token) return false;
    ...
    const updateSettings = async (name?: string, isPublic?: boolean): Promise<boolean> => {
        if (!token) return false;
    ...
    const checkInWishlist = useCallback((productId: string): boolean => {
